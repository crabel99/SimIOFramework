...
inline void SERCOM::setSlaveTxnWIRE(SercomTxn* txn)
{
	_wire.slaveTxn = txn;
	if (isSlaveWIRE() && !_wire.active)
		setTxnWIRE(txn);
}
...
inline void SERCOM::dmaRxCallbackWIRE(Adafruit_ZeroDMA* dma)
{
	...
	// Transfer-complete and suspend can become pending together at an exact
	// descriptor boundary. Completion owns retirement in that case; do not let
